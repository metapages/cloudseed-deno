...
_help:
    ...
    if [ -f /.dockerenv ]; then
        ...
        deno run --unstable --allow-all {{DENO_SOURCE}}/cloudseed/docker/docker_mount.ts --mount-ssh=true --mount-git-config=true  --user=root --image={{DOCKER_IMAGE_PREFIX}}{{DOCKER_IMAGE_NAME}}:{{DOCKER_TAG}} --context="." --dockerfile=Dockerfile --command=bash;
    fi
